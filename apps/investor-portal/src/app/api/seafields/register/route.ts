  ...
  try {
    ...
    const priceRows = d.price_preferences
      ? Object.entries(d.price_preferences)
          .map(
            ([lid, range]) =>
              `<tr><td style="padding:2px 12px;color:#666">${lid.replace("L", "Lot ")}</td><td style="padding:2px 12px">${range}</td></tr>`
          )
          .join("")
      : "";
    const referrerRow =
    ...
    await resend.emails.send({
      ...
      subject: `Seafields ROI: ${d.first_name} ${d.last_name} — ${lotList}`,
      html: `
        ...
        <table style="border-collapse:collapse;font-family:sans-serif;font-size:14px">
          ...
          ${d.interest_type ? `<tr><td style="padding:4px 12px;color:#666">Interest</td><td style="padding:4px 12px;font-weight:bold">${d.interest_type}</td></tr>` : ""}
        ...
        <p style="margin-top:16px;font-size:12px;color:#999">Seafields Estate ROI — F2K Fund Tokenisation Project</p>
      `,
    });
      ...
      html: `
        <div style="max-width:600px;font-family:sans-serif">
          ...
          <div style="padding:32px;background:#FFFFFF">
            ...
            ${d.interest_type ? `<p style="font-size:14px;color:#4A5568;line-height:1.6">Interest type: <strong>${d.interest_type}</strong></p>` : ""}
            <p style="font-size:14px;color:#4A5568;line-height:1.6">
