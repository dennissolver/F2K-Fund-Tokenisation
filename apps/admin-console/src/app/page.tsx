...
export default async function AdminDashboard() {
  ...
  return (
    <div>
      ...
      {(() => {
        const FUND_TARGET = 600_000_000;
        const pct = FUND_TARGET > 0 ? Math.min((totalAum / FUND_TARGET) * 100, 100) : 0;
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border mb-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-navy text-white">
                $600M Wholesale Raise
              </span>
            </div>
            <div className="flex justify-between text-sm mb-1">
              <span className="text-gray-600">{fmt(totalAum)} raised</span>
              <span className="text-gray-500">{fmt(FUND_TARGET)} target</span>
            </div>
            ...
            <div className="flex justify-between text-xs text-gray-400 mt-1">
              ...
              <span>{fmt(FUND_TARGET - totalAum)} remaining</span>
            </div>
          </div>
        );
      })()}
